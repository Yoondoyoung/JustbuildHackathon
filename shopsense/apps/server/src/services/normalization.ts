import type { ProductItem, Shipping } from "../types/search";
...
export function normalize(source: ProductSource, rawItem: Record<string, unknown>): ProductItem {
  ...
  const url = str(rawItem.product_page_url ?? rawItem.link ?? rawItem.product_link ?? rawItem.url ?? "#");
  ...
  let sourceId = str(rawItem.product_id ?? rawItem.us_item_id ?? rawItem.asin ?? rawItem.id ?? "").trim() || undefined;
