...
import { timer, nowISO } from '../utils/time';
...
export async function aggregateSearch(
  ...
  if (cached) {
    logger.info({ requestId: ctx.requestId, query: req.query, cache: 'hit' });
    return cached;
  }
  ...
  const limit = req.maxResultsPerSource;

  const [amazonResult, googleResult, walmartResult] = await Promise.allSettled([
    amazonConnector.search(req.query, req.locale, limit, filters, connectorCtx),
    googleConnector.search(req.query, req.locale, limit, filters, connectorCtx),
    walmartConnector.search(req.query, req.locale, limit, filters, connectorCtx),
  ]);
  ...
  if (amazonResult.status === 'rejected') {
    ...
    logger.warn({ requestId: ctx.requestId, source: 'amazon', err: amazonResult.reason });
  }
  if (googleResult.status === 'rejected') {
    ...
    logger.warn({ requestId: ctx.requestId, source: 'google', err: googleResult.reason });
  }
  if (walmartResult.status === 'rejected') {
    ...
    logger.warn({ requestId: ctx.requestId, source: 'walmart', err: walmartResult.reason });
  }
  ...
  merged = sortProducts(merged, req.query, req.sort);
